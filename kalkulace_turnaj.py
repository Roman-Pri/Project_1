...
def spocitat_naklady():
    try:
        ...
        zdravotni_zabezpeceni = int(
            entry_pocet_zdravotni_zabezpeceni.get()
        )
        ...
        zapasy_na_skupinu = (
            zapasy_na_tym_skupina * pocet_tyms_ve_skupine // 2
        )

        # Výpočet celkového počtu zápasů na turnaj (pro všechny skupiny)

        celkovy_pocet_zapasu_na_turnaj = (
            zapasy_na_skupinu * pocet_skupin
        ) + zapasy_playoff

        # Výpočet počtu hráčů a brankářů

        celkovy_pocet_hracu = (
            pocet_hracu * pocet_tyms_ve_skupine * pocet_skupin
        )
        celkovy_pocet_brankaru = (
            pocet_brankaru * pocet_tyms_ve_skupine * pocet_skupin
        )
        ...
        celkova_cena_treneri = (
            cena_za_trener * celkovy_pocet_zapasu_na_turnaj
        )
        ...
        celkova_cena_zdravotnik = (
            cena_za_zdravotnik * celkovy_pocet_zapasu_na_turnaj
        )
        cena_obcerstveni_zapas = cena_za_obcerstveni * (
            2 * pocet_hracu
            + 2 * pocet_brankaru
            + rozhodci_na_zapas
            + POCET_TRENERU_NA_ZAPAS
            + pocet_organizatoru
            + zdravotni_zabezpeceni
        )
        celkova_cena_obcerstveni = (
            cena_obcerstveni_zapas * celkovy_pocet_zapasu_na_turnaj
        )
        celkova_cena_vybaveni = cena_za_vybaveni * (
            celkovy_pocet_hracu + celkovy_pocet_brankaru
        )
        ...
        minimalni_cena_za_jednoho_hrace = celkova_cena_za_turnaj / (
            celkovy_pocet_hracu + celkovy_pocet_brankaru
        )
        ...
        ceny_s_marzi = [
            round(minimalni_cena_za_jednoho_hrace * (1 + m / 100), 2)
            for m in marze
        ]
        ...
        text_widget = tk.Text(
            vysledek_okno, height=38, width=50, wrap=tk.WORD
        )
        ...
        for i, (popis, hodnota) in enumerate(zip(popisy, hodnoty)):
            if popis == "Celková cena za celý turnaj:":
                ...
                text_widget.insert(
                    tk.END,
                    f"{popis.ljust(max_délka_popisu)} {str(hodnota).rjust(10)} Kč\n",
                    "highlight",
                )  # Zvýraznění
                text_widget.insert(tk.END, f"{50*'-'}\n")
            elif popis in [
                ...
                text_widget.insert(
                    tk.END,
                    f"{popis.ljust(max_délka_popisu)} {str(hodnota).rjust(10)}\n",
                )
            else:
                # Zobrazení ceny s "Kč"
        ...
        text_widget.insert(
            tk.END,
            f"Cena na jednoho hráče:{minimalni_cena_za_jednoho_hrace:24.0f} Kč\n",
            "highlight2",
        )
        for popis, ceny in zip(popisy_marze, ceny_s_marzi):
            text_widget.insert(
                tk.END,
                f"{popis.ljust(max_délka_popisu)} {str(round(ceny)).rjust(10)} Kč\n",
                "highlight2",
            )
        text_widget.insert(tk.END, f"{50*'-'}\n")
        ...
        for marze, vydelek in vysledky:
            text_widget.insert(
                tk.END,
                f"Výdělek s marží {marze:3}%: {vydelek:24.0f} Kč\n",
                "highlight2",
            )
...
frame_financni.grid(
    row=0, column=0, padx=20, pady=20, sticky="n", ipadx=10, ipady=10
)

frame_pocetni = tk.Frame(root, bg="#e6e6e6", bd=5, relief="solid")
frame_pocetni.grid(
    row=0, column=1, padx=20, pady=20, sticky="n", ipadx=10, ipady=10
)

# Finanční parametry

label_cena_led = tk.Label(
    frame_financni,
    text="Cena za pronájem 1 h ledu (Kč):",
    anchor="w",
    bg="#e6e6e6",
    font=("Arial", 10),
)
...
label_cena_trener = tk.Label(
    frame_financni,
    text="Cena za 1 x trenér / zápas (Kč):",
    anchor="w",
    bg="#e6e6e6",
    font=("Arial", 10),
)
...
label_cena_rozhodci = tk.Label(
    frame_financni,
    text="Cena za 1 x rozhodčí / zápas (Kč):",
    anchor="w",
    bg="#e6e6e6",
    font=("Arial", 10),
)
...
label_cena_zdravotnik = tk.Label(
    frame_financni,
    text="Cena za zdravotní zabezpečení / zápas (Kč):",
    anchor="w",
    bg="#e6e6e6",
    font=("Arial", 10),
)
...
label_cena_vybaveni = tk.Label(
    frame_financni,
    text="Cena za vybavení / hráč (Kč):",
    anchor="w",
    bg="#e6e6e6",
    font=("Arial", 10),
)
...
label_cena_oceneni = tk.Label(
    frame_financni,
    text="Náklady na ceny (Kč):",
    anchor="w",
    bg="#e6e6e6",
    font=("Arial", 10),
)
...
label_cena_obcerstveni = tk.Label(
    frame_financni,
    text="Cena za občerstvení / osobu / zápas (Kč):",
    anchor="w",
    bg="#e6e6e6",
    font=("Arial", 10),
)
...
label_cena_cestovne = tk.Label(
    frame_financni,
    text="Náklady na cestovné (Kč):",
    anchor="w",
    bg="#e6e6e6",
    font=("Arial", 10),
)
...
label_cena_marketing = tk.Label(
    frame_financni,
    text="Náklady na marketing (Kč):",
    anchor="w",
    bg="#e6e6e6",
    font=("Arial", 10),
)
...
label_cena_merch = tk.Label(
    frame_financni,
    text="Náklady na merchandise (Kč):",
    anchor="w",
    bg="#e6e6e6",
    font=("Arial", 10),
)
...
label_pocet_hracu = tk.Label(
    frame_pocetni,
    text="Počet hráčů v týmu:",
    anchor="w",
    bg="#e6e6e6",
    font=("Arial", 10),
)
...
label_pocet_brankaru = tk.Label(
    frame_pocetni,
    text="Počet brankářů v týmu:",
    anchor="w",
    bg="#e6e6e6",
    font=("Arial", 10),
)
...
label_pocet_tyms_ve_skupine = tk.Label(
    frame_pocetni,
    text="Počet týmů ve skupině:",
    anchor="w",
    bg="#e6e6e6",
    font=("Arial", 10),
)
label_pocet_tyms_ve_skupine.grid(
...
label_zapasy_na_tym_skupina = tk.Label(
    frame_pocetni,
    text="Počet zápasů na tým ve skupině:",
    anchor="w",
    bg="#e6e6e6",
    font=("Arial", 10),
)
label_zapasy_na_tym_skupina.grid(
...
label_zapasy_playoff = tk.Label(
    frame_pocetni,
    text="Počet zápasů na tým v playoff:",
    anchor="w",
    bg="#e6e6e6",
    font=("Arial", 10),
)
...
label_pocet_treneru = tk.Label(
    frame_pocetni,
    text="Počet trenérů na turnaj:",
    anchor="w",
    bg="#e6e6e6",
    font=("Arial", 10),
)
...
label_pocet_rozhodci = tk.Label(
    frame_pocetni,
    text="Počet rozhodčích na zápas:",
    anchor="w",
    bg="#e6e6e6",
    font=("Arial", 10),
)
...
label_pocet_organizatoru = tk.Label(
    frame_pocetni,
    text="Počet organizátorů:",
    anchor="w",
    bg="#e6e6e6",
    font=("Arial", 10),
)
label_pocet_organizatoru.grid(
...
label_pocet_zdravotni_zabezpeceni = tk.Label(
    frame_pocetni,
    text="Počet zdravotníků:",
    anchor="w",
    bg="#e6e6e6",
    font=("Arial", 10),
)
label_pocet_zdravotni_zabezpeceni.grid(
    row=9, column=0, padx=10, pady=5, sticky="w"
)
entry_pocet_zdravotni_zabezpeceni = tk.Entry(
    frame_pocetni, font=("Arial", 10)
)
entry_pocet_zdravotni_zabezpeceni.grid(row=9, column=1, padx=10, pady=5)

# Tlačítko pro výpočet nákladů

button_vypocitat = tk.Button(
    root,
    text="Spočítat náklady",
    command=spocitat_naklady,
    bg="#4CAF50",
    fg="white",
    font=("Arial", 12),
)
