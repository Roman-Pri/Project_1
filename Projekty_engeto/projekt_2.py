...
def evaluate_guess(secret_number, guess_digits):
    ...
    cows = sum(1 for i in range(4) if guess_digits[i] in secret_number and secret_number[i] != guess_digits[i])
    return bulls, cows
...
def run_new_game():
    ...
    text_label = tk.Label(window, text=opening_text, font=("Arial", 20), background="green", justify="left")
    text_label.place(x=10, y=10)
    
    # Time label
    time_label = tk.Label(window, text="Game Time - 0:00", font=("Arial", 20), background="green", justify="right")
    ...
    tk.Label(window, text="Insert the number and press Enter:", font=("Arial", 16), background="green").place(x=635, y=580)
    
    # Register to display all previous guesses and results + scrollbar
    guesses_register = tk.Listbox(window, font=("Arial", 16), width=50, height=20)
    guesses_register.place(x=640, y=62)
    scrollbar = tk.Scrollbar(window, orient="vertical", command=guesses_register.yview)
    ...
    def check_guess(event=None):
        ...
        guesses_register.insert(tk.END, f"Guess {game_state['guesses']}: {guess} >>> {bulls} bull{'s' if bulls != 1 else ''}, {cows} cow{'s' if cows != 1 else ''}")
        guesses_register.yview_scroll(1, "units")
        
        if bulls == 4:
            game_time = stop_stopwatch(time_label, stopwatch)
            guesses_register.insert(tk.END, f"Correct, you've guessed the right number in {game_state['guesses']} guesses!\n")
            guesses_register.yview_scroll(1, "units")
            guesses_register.insert(tk.END, f"Your game time is {game_time}.\n")
            guesses_register.yview_scroll(1, "units")
            ...
            tk.Button(window, text="New Game", font=("Arial", 20), width=15, height=2, command=run_new_game).place(x=200, y=300)
            tk.Button(window, text="End Game", font=("Arial", 20), width=15, height=2, command=window.quit).place(x=200, y=400)
...
tk.Label(window, text="Welcome to BULLs & COWs game", font=("Arial", 40), background="green").pack(pady=10)
tk.Button(window, text="NEW GAME", font=("Arial", 20), width=15, height=2, command=run_new_game).place(x=525, y=200)
